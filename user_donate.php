...
<body>
    ...
    <!-- Create Donation Post Modal -->
    <div class="modal fade" id="PostDonation" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h5 class="modal-title w-100" id="exampleModalLabel">Donation Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <form action="addDonation.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <h6 class="text-center">Donation Description</h6>
                        <div class="form-group">
                            <label for="inputAnimalName">Reason for Donation</label>
                            <input required type="text" class="form-control" id="addDonationDescription" name="addDonationDescription" placeholder="Enter Reason for Donation">
                        </div>
                        <div class="form-group">
                            <label for="inputAnimalName">Amount Needed</label>
                            <input required type="number" class="form-control" id="addDonationAmount" name="addDonationAmount" placeholder="Enter Amount Needed">
                        </div>
                        <input type="hidden" id="addDonationStatus" name="addDonationStatus" value="Ongoing">
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" name="postDonationButton" class="btn btn-1">Post Donation</button>
                    </div>
    ...
    <!-- Send Donation Modal -->
    <div class="modal fade" id="MakeDonation" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h5 class="modal-title w-100" id="exampleModalLabel">Send Donation</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="sendDonation.php" method="POST">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="inputDonorName">Donor Name (Optional):</label>
                            <input type="text" class="form-control" name="addDonorName" placeholder="Enter Your Name (Optional)">
                        </div>
                        <div class="form-group">
                            <label for="inputGender">Choose Payment Method:</label>
                            <select required type='text' class="form-control" name="addPaymentMethod">
                        ...
                        <div class="form-group">
                            <label for="inputDonorName">How Much Would You Like to Donate?:</label>
                            <input required type="number" class="form-control" name="addAmountDonated" placeholder="Enter Amount">
                        </div>
                        <div class="form-group">
                            <label for="inputDonorName">Message (Optional):</label>
                            <input type="text" class="form-control" name="addDonorMessage" placeholder="Enter Message" style="height:100px;">
                        </div>
                    </div>
                    <input type="hidden" name="animalID" id="dataid" value="" />
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" name="SendDonation" class="btn btn-1">Be A Hero!</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Title -->
    <h1 class="text-center">"Give a little, help a lot - every donation can make a big impact in an animal's life."</h1>

    <!-- If user is logged in -->
    <?php if (isset($_SESSION['auth_user'])) : ?>
        <h6 class="text-center"><button class="btn btn-1" data-toggle="modal" data-target="#PostDonation">Post an Animal who needs Donation? Click Here!</button></h6>
        <!-- If user is not loggedin -->
    <?php else : ?>
        <h6 class="text-center">Want to post an Animal who needs Donation? <a href="" data-toggle="modal" data-target="#SignIn">Sign in Now.</a></h6>
    <?php endif; ?>
    ...
    <div class="container-fluid mt-3 py-3">
        <div class="row">
            ...
            $sql = "SELECT 
                ...
                d.animal_donation_id AS AnimalID,
                d.amount_needed AS Needed,
                d.amount_reached AS Reached,
                ((d.amount_reached / d.amount_needed) * 100) AS AmountPercentage,
                d.donation_description AS Description,
                d.date_listed AS PostedOn,
                d.donation_status AS Status
            FROM donation_listing as d
            JOIN animal_identification as a
                ON a.animal_id = d.animal_donation_id
            JOIN user_details as u
                ON u.user_id = d.user_donation_id
            WHERE d.donation_status = 'Ongoing';";
            $result = mysqli_query($con, $sql);
            $check_result = mysqli_num_rows($result) > 0;

            if ($check_result) {
                while ($row = mysqli_fetch_array($result)) {
                    $animal_donation_id = $row['AnimalID'];
            ?>
                    <div class="col-xl-6">
                        <div class="card mb-3 m-1 shadow-lg" style="background-color:rgba(255, 156, 0, );">
                            <div class="row no-gutters">
                                <div class="col-md-5">
                                    <img src="./uploads/animals/<?php echo $row['Image']; ?>" class="rounded mx-auto" style="height: 100%; width:100%; object-fit:fill; object-position: 50% 50%;" alt="...">
                                </div>
                                <div class="col-md-7">
                                    <div class="card-body">
                                        <h5 class=" card-title"><?php echo $row['Name']; ?></h5>
                                        <p class="card-text">
                                            <b>Posted By:</b> <?php echo $row['Username']; ?> | <?php echo $row['Contact']; ?> | <?php echo $row['Email']; ?><br>
                                        ...
                                        <label for="amountNeeded">Amount Needed: <?php echo $row['Needed']; ?></label>
                                        <div class="progress" style="height: 30px;">
                                            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width:<?php echo $row['AmountPercentage']; ?>%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"><b>Amount Reached: <?php echo $row['Reached']; ?></b></div>
                                        </div>
                                        <button class="btn btn-1 btn-block mt-1" data-id="<?php echo $row['AnimalID']; ?>" onclick="$('#dataid').val($(this).data('id')); $('#MakeDonation').modal('show');">Send Donation</button>
                                    </div>
                                </div>
                            </div>
                        </div>
